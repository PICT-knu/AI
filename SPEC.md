...
Python FastAPI 기반의 **복합 AI 플랫폼 서버**. 이력서 최적화·초안 생성·소재 추출·챗봇 교정·채용공고 매칭·공고 상세 분석을 핵심 기능으로 하며, 다층 할루시네이션 방지 파이프라인을 내장한다. BE1(Spring)이 HTTP로 호출하는 내부 전용 서버이다.
...
- **구현 파일**: `services/resume_service.py` (`fix_resume`)
...
- **엔드포인트**: `POST /resume/pdf/extract`, `POST /resume/text/extract`
...
| `/text/extract` | 자유 텍스트 (JSON, Notion 또는 직접 입력) | 있음 (회의록·피드백·코드 덤프 등 필터) |
...
- **엔드포인트**: `POST /resume/generate`
- **구현 파일**: `services/resume_service.py` (`generate_resume`)
- 2.1과 동일한 팩트 마스킹+Planner+Generator+Verifier 파이프라인 사용. Generator 프롬프트에 `user_profile` 추가.
...
### 2.6 공고 상세 분석 (Job Detail Analysis — Stateless)

- **엔드포인트**: `POST /jobs/detail-analysis`
- **구현 파일**: `services/job_service.py` (`analyze_job_detail`)
- **LLM**: OpenRouter (`JOB_ANALYSIS_MODEL`, 미설정 시 `OPENROUTER_MODEL`) — httpx 직접 호출
- **Temperature**: 0.2

- 공고 원문(`description`) 한 필드를 분석해 `main_tasks`(주요 업무)·`qualifications`(자격 요건)·`benefits`(복지·혜택) 세 배열로 정리. 공고 상세 조회 페이지 요약 카드용.
- OpenRouter `response_format` JSON Schema strict 모드로 구조화 출력 강제 (소재 추출과 동일 방식).
- 공고에 명시된 내용만 추출하고 지어내지 않음 (할루시네이션 방지). 섹션당 최대 6개로 제한.
- 빈/공백 `description`은 LLM 호출 없이 빈 배열 반환. AI 처리 실패 시 502.
...
- **LLM 타임아웃**: `asyncio.wait_for`로 LLM 호출을 감싸 기본 90초 제한. `LLM_TIMEOUT_SECONDS` 환경변수로 변경 가능. 초과 시 `asyncio.TimeoutError` → HTTP 504 반환.
...
> **현재 미구현**: 스트리밍은 현재 BE1 연동 범위 밖이다. 모든 엔드포인트는 전체 응답 완성 후 JSON을 반환한다.
...
> **현재 미구현**: AI 서버는 DB 직접 접근 또는 내부 마이크로서비스 호출을 수행하지 않는다. 모든 데이터는 BE1이 페이로드에 포함하여 전달한다. 에이전트 툴 호출은 현재 BE1 연동 범위 밖이다.
...
POST /resume/text/extract
  - Notion 또는 직접 입력 텍스트에서 소재 카드 추출 (JSON)
  - Body: { text: "Notion 또는 직접 작성 본문" }
  - Response: { materials: [{ title, summary, material_type }] }
...
POST /jobs/detail-analysis
  - 공고 상세 분석 (공고 상세 페이지용, 무상태)
  - Body: { job_posting_id?, company_name?, title?, description: string }
  - Response: { main_tasks: [...], qualifications: [...], benefits: [...] }
  - OpenRouter JSON Schema strict, 섹션당 최대 6개. 실패 시 502
```
...
├── routers/
│   ├── __init__.py                # resume_router, matching_router, jobs_router export
│   ├── resume.py                  # /resume/* 엔드포인트 5개 정의
│   ├── matching.py                # /match/top10 엔드포인트 정의
│   └── jobs.py                    # /jobs/detail-analysis 엔드포인트 정의
├── services/
...
│   ├── resume_service.py          # fix_resume, chat_resume, generate_resume 구현
│   │                              # 팩트 마스킹 + Planner + Generator + Verifier 파이프라인 포함
│   ├── matching_service.py        # 3단계 매칭 파이프라인 (사전필터 → 임베딩 → LLM 스코어링)
│   ├── pdf_service.py             # PDF·텍스트 소재 추출 (OpenRouter httpx 직접 호출)
│   └── job_service.py             # 공고 상세 분석 (OpenRouter JSON Schema strict httpx 호출)
├── models/
...
│   ├── pdf.py                     # MaterialType(Enum), ExtractedMaterial, PdfExtractResponse, TextExtractRequest
│   └── job.py                     # JobDetailAnalysisRequest, JobDetailAnalysisResponse
└── utils/
    ├── __init__.py
...
| `OPENROUTER_API_KEY` | **필수** | — | OpenRouter API 키 (소재 추출·매칭 임베딩에 필수, 서버 시작 시 검증) |
...
| `TEXT_EXTRACT_MODEL` | 선택 | PDF_EXTRACT_MODEL 값 | 텍스트 추출 전용 모델 (Notion·직접입력 공용) |
| `JOB_ANALYSIS_MODEL` | 선택 | OPENROUTER_MODEL 값 | 공고 상세 분석(`/jobs/detail-analysis`) 전용 모델 |
| `LLM_TIMEOUT_SECONDS` | 선택 | `90` | LLM 호출 타임아웃(초). 초과 시 504 반환 |
