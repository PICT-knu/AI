...
아래 텍스트(Notion 페이지 본문 등 자유 형식)를 분석하여 아래 규칙에 따라 소재를 최대한 적극적으로 추출하십시오.

[소재 유형]
- EXPERIENCE: 직장 경력, 인턴십 등 실제 근무 이력
- PROJECT: 팀/개인 프로젝트, 캡스톤, 오픈소스 기여 등 작업 결과물
- SKILL: 프로그래밍 언어, 프레임워크, 자격증, 어학 능력 등 보유 역량
- EDUCATION: 학력 (학교, 전공, 수료 과정)
- OTHER: 수상 이력, 봉사활동 등 기타

[추출 판단 기준]
- 작업 중인 프로젝트 기록, 회의록, 개발 노트, 설계 문서 등은 모두 PROJECT로 추출하십시오.
- 사용한 기술·도구·언어가 언급되면 SKILL로 추출하십시오.
- 맡은 역할, 한 일, 의사결정, 트러블슈팅 등 활동이 있으면 적극 추출하십시오.
- 빈 배열을 반환하는 경우는 오직 다음에 한정합니다:
  순수 연락처 목록, 단순 날짜·일정표, 의미 없는 메모(예: "TODO", "TBD") 뿐인 경우.

[규칙]
1. 텍스트에 명시된 내용만 추출하고 내용을 지어내지 마십시오.
2. 각 소재는 독립 항목 단위로 분리하십시오.
3. title은 30자 이내 간결한 제목으로 작성하십시오.
4. summary는 텍스트에서 파악한 활동·기여·결과를 2~4문장으로 요약하십시오. 내용이 부족하면 null로 작성하십시오.
5. 소재가 전혀 없는 경우만 빈 배열로 반환하십시오."""
...
async def extract_materials_from_pdf(pdf_bytes: bytes, filename: str) -> PdfExtractResponse:
    ...
    model = os.getenv("PDF_EXTRACT_MODEL", "google/gemini-2.5-flash-lite")
